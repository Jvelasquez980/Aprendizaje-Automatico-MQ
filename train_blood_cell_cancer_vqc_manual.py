...
DEFAULT_OUTPUT_DIR = Path("artifacts") / "blood_cell_cancer_quantum" / "vqc_manual"
...
def canonicalize_amplitudes(amplitudes: np.ndarray) -> np.ndarray:
    ...
    if not np.all(np.isfinite(vec)):
        raise ValueError("Se encontraron amplitudes no finitas durante la canonizacion.")
    ...
    # Qiskit puede aceptar vectores "casi" normalizados en raw_feature_vector,
    # pero luego fallar dentro de initialize() por tolerancias mas estrictas.
    # Reajustamos un componente para que la suma de cuadrados quede en 1 con mas estabilidad.
    if vec.size > 1:
...
class ManualAmplitudeVQC:
    def __init__(self, num_qubits: int, reps: int, verbose: bool = True):
        ...
        if verbose:
            print(
                f"Construyendo ansatz real_amplitudes para {num_qubits} qubits con reps={reps} y entanglement='linear'...",
                flush=True,
            )
    ...
    def predict_proba_one(self, amplitudes: np.ndarray, weights: np.ndarray) -> np.ndarray:
    ...
    def predict_proba(self, X: np.ndarray, weights: np.ndarray) -> np.ndarray:
        outputs = np.zeros((X.shape[0], 4), dtype=np.float64)
        for idx, amplitudes in enumerate(X):
            outputs[idx] = self.predict_proba_one(amplitudes, weights)
        return clip_probabilities(outputs)
...
def train_manual_vqc(
    ...
    history_path = output_dir / f"manual_vqc_{num_qubits}q_history.csv"

    model = ManualAmplitudeVQC(num_qubits=num_qubits, reps=reps, verbose=verbose)
    if verbose:
    ...
    def callback(nfev: int, weights: np.ndarray, objective_value: float, step_size: float, accepted: bool) -> None:
        ...
        if verbose:
            print(
                f"[Iter {iteration}] nfev={nfev} objective={float(objective_value):.8f} step={float(step_size):.8f}",
                flush=True,
            )
    ...
    if verbose:
        print(
            f"Iniciando entrenamiento manual VQC: qubits={num_qubits}, reps={reps}, batch_size={batch_size}, maxiter={maxiter}",
            flush=True,
        )
    start = time.time()
    ...
    weights_path = output_dir / f"manual_vqc_{num_qubits}q_weights.npy"
    ...
    config = {
        ...
        "measurement": "Probabilidades marginales sobre los qubits [0, 1] para representar 4 clases.",
    ...
    (output_dir / f"manual_vqc_{num_qubits}q_config.json").write_text(json.dumps(config, indent=2), encoding="utf-8")
    ...
    (output_dir / f"manual_vqc_{num_qubits}q_results.json").write_text(json.dumps(results, indent=2), encoding="utf-8")
    return results


def parse_args():
    parser = argparse.ArgumentParser(
        description="Entrena un flujo manual de VQC con Qiskit y amplitude encoding sin usar el envoltorio VQC."
    )
    ...
    parser.add_argument("--num-qubits", type=int, choices=[8, 16], default=16, help="Numero de qubits del flujo manual.")
    ...
    parser.add_argument("--batch-size", type=int, default=8, help="Tamano de minibatch para la funcion objetivo.")
    ...
    parser.add_argument(
        "--eval-limit-per-class",
        type=int,
        default=64,
        help="Limite estratificado por clase para evaluar mas rapido. Usa 0 para evaluar todo el split.",
    )
    parser.add_argument("--quiet", action="store_true", help="Desactiva los mensajes de progreso en consola.")
...
def main():
    ...
    if verbose:
        ...
        print(
            f"Configuracion solicitada: num_qubits={args.num_qubits}, reps={args.reps}, maxiter={args.maxiter}, batch_size={args.batch_size}",
            flush=True,
        )
